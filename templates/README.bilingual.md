...
- Keep section order aligned where possible, but remove sections absent from both files rather than adding empty headings. Both files must always contain corresponding, non-empty, verified Quick Start sections; if the first-use path is missing, ask the user instead of finalizing either file.
